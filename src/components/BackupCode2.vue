        ...
        <div class="search">
            ...
            <form @submit.prevent="handleSubmit" class="input-container">
                <input v-model="videoURL" type="text" name="VideoURL" placeholder="Paste URL for a Video"
                    class="custom-field">
                <button @click="handleMP4" class="searchbtn">Get Video</button>
...
export default {
    ...
    methods: {
        ...
        handleMP4() {
            this.video_btn_clicked = true;
        },
        async handleSubmit() {
            // Video Button Clicked
            if (this.video_btn_clicked === true) {
                ...
                this.video_btn_clicked = false;
            }
            // Mp3 Button Clicked
            else if (this.mp3_btn_clicked === true) {
                console.log('mp3 button clicked!')
                this.spinnerLoad_mp3 = true
                if (this.videoURL === undefined || this.videoURL === "" || this.videoURL === null) {
                    this.successCheck_mp3 = false
                    this.failureMessage = "Please enter a valid Youtube URL!"
                } else {
                    this.successCheck_mp3 = true
                    ...
                    const options = {
                        ...
                        headers: {
                            'X-RapidAPI-Key': process.env.VUE_APP_API_KEY,
                            'X-RapidAPI-Host': process.env.VUE_APP_API_HOST_2,
                        }
                    };

                    const fetchAPI = await fetch(`https://youtube-mp36.p.rapidapi.com/dl?id=${this.videoID}`, options)
                    ...
                    if (fetchResponse.status === "ok") {
                        ...
                        this.successCheck_mp3 = true
                        this.videoTitle = fetchResponse.title
                        this.videoLength = this.convert(fetchResponse.duration)
                        ...
                        //Assign Formats to Variables
                        this.videoFormat_mp3_Link = fetchResponse.link

                    } else {
                        //Catch any other Error
                        this.successCheck_mp3 = false
                        this.failureMessage = fetchResponse.msg

                    }
                }
                //Re-set Spinner
                this.spinnerLoad_mp3 = false;
                this.mp3_btn_clicked = false;
            }
